...
contract ContractDataLayout is LibraryLock {
    ...
    struct userSwapStruct {
        uint256 BUSDProvided;
        bool LPClaimed;
    }
    mapping(address => userSwapStruct) public userSwap;
    uint256 public totalBUSDProvided;
...
contract LuckV3 is Context, IERC20, Ownable, ContractDataLayout, Proxiable {
    ...
    function proxyConstruct() public {
        require(!initialized);
    ...
    /** @notice Allows the owner to immediately update the contract logic.
      ...
      */
    function ownerUpdate(address newCode) public onlyOwner delegatedOnly {
    ...
    function tokenSwap(uint256 amount, address token) public {
        ...
        userSwap[msg.sender].BUSDProvided = userSwap[msg.sender].BUSDProvided.add(postBal.sub(preBal));
        totalBUSDProvided = totalBUSDProvided.add(postBal.sub(preBal));
        //deposit event
    }

    function initializeLiquidity() public onlyOwner {
        IERC20(BUSD).approve(address(uniswapV2Router), IERC20(BUSD).balanceOf(address(this)));
        IERC20(address(this)).approve(address(uniswapV2Router), IERC20(address(this)).balanceOf(address(this)));
        ...
        require(IERC20(uniswapV2Pair).balanceOf(tokenStorage) > 0);
        LPCreated = IERC20(uniswapV2Pair).balanceOf(tokenStorage);

        //pool creation event
    }

    function claimLP() public {
        require(!userSwap[msg.sender].LPClaimed);
        uint256 claimable = userSwap[msg.sender].BUSDProvided.mul(LPCreated).div(totalBUSDProvided);
        ERCStorage(tokenStorage).sendTokens(uniswapV2Pair, msg.sender, claimable);
        userSwap[msg.sender].LPClaimed = true;
    ...
    //manually add to the normal or jackpot draw
    //true for normal, false for jackpot
    function manualDrawAdd(bool normal, uint256 amount) public {
        IERC20(BUSD).transferFrom(msg.sender, address(this), amount);
        if (normal) {
            luckyDrawAmount = luckyDrawAmount.add(amount);
        } else {
            jackpotAmount = jackpotAmount.add(amount);
        }
    }
